...
class TestData(unittest.TestCase):
    def test_cities(self):
        cities = data.cities
        self.assertEqual(
            cities.shape, (302, 3), "The shape of cities should be (302, 3)."
        )
    ...
    def test_neighbors(self):
        ...
        for i in range(302):
            self.assertTrue(
                neighbors[i] is not None and len(neighbors[i]) > 0,
    ...
    def test_triangle2faces(self):
        ...
        self.assertEqual(
            len(triangle2faces), 80, "The number of triangle faces should be 80."
        )

    def test_pentagon2faces(self):
        pentagon2faces = data.pentagon2faces
        self.assertEqual(
            len(pentagon2faces), 12, "The number of pentagon faces should be 12."
        )
    ...
    def test_indexer(self):
        ...
        self.assertFalse(
            set([i for i in range(302)]) - set(indexer.values()),
            "Each index should be in the indexer.",
        )
        for i in range(60):
            self.assertEqual(
                i, indexer[tuple((i,))], "Each index should be in the indexer."
            )
        for i in range(15):
            j = (i + 1) % 15
            self.assertTrue(
                (i, j) in indexer, "Each midpoint should be in the indexer."
            )
            self.assertTrue(
                (15 + i, 15 + j) in indexer, "Each midpoint should be in the indexer."
            )
            self.assertTrue(
                (30 + i, 30 + j) in indexer, "Each midpoint should be in the indexer."
            )
            self.assertTrue(
                (45 + i, 45 + j) in indexer, "Each midpoint should be in the indexer."
            )
