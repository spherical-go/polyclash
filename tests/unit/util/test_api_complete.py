...
class TestAPIServerCommunication:
    ...
    def test_connect_success(self, mock_post):
        ...
        mock_post.assert_called_once_with(
            "http://example.com/sphgo/new", json={"token": "game123"}
        )
    ...
    def test_connect_connection_error(self, mock_post):
        ...
        # Call connect and check error
        with pytest.raises(
            ValueError, match="Server not reachable when we start the game"
        ):
            connect("http://example.com", "game123")
    ...
    def test_get_game_status_both(self, mock_post):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"status": {"black": True, "white": True}}
        mock_post.return_value = mock_response
        ...
        # Check request
        mock_post.assert_called_once_with(
            "http://example.com/sphgo/joined_status", json={"token": "token123"}
        )
    ...
    def test_get_game_status_black(self, mock_post):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"status": {"black": True, "white": False}}
        mock_post.return_value = mock_response
    ...
    def test_get_game_status_white(self, mock_post):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"status": {"black": False, "white": True}}
        mock_post.return_value = mock_response
    ...
    def test_get_game_status_neither(self, mock_post):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"status": {"black": False, "white": False}}
        mock_post.return_value = mock_response
    ...
    def test_joined_status(self, mock_get_game_status):
        ...
        mock_get_game_status.assert_called_once_with(
            "joined", "http://example.com", "token123"
        )
        assert result == "Both"
    ...
    def test_join_success(self, mock_post):
        ...
        # Check request
        mock_post.assert_called_once_with(
            "http://example.com/sphgo/join", json={"token": "token123", "role": "black"}
        )
    ...
    def test_ready_status(self, mock_get_game_status):
        ...
        mock_get_game_status.assert_called_once_with(
            "ready", "http://example.com", "token123"
        )
        assert result == "Both"
    ...
    def test_play_connection_error(self, mock_post):
        ...
        # Call play and check error
        with pytest.raises(
            ValueError, match="Server not reachable when we play the game"
        ):
            play("http://example.com", 1, [0])
    ...
    def test_close_success(self, mock_post):
        ...
        # Check request
        mock_post.assert_called_once_with(
            "http://example.com/sphgo/close", json={"token": "game123"}
        )
    ...
    def test_close_connection_error(self, mock_post):
        ...
        # Call close and check error
        with pytest.raises(
            ValueError, match="Server not reachable when we close the game"
        ):
            close("http://example.com")
