...
from polyclash.server import (
    app, socketio, storage, secret_key, server_token, valid_plays,
    player_join_room, viewer_join_room, player_ready, delayed_start,
    api_call, index, list_games, new, joined_status, join, ready_status,
    ready, cancel, close, play, on_join, on_ready
)
...
def prevent_redis_calls():
    ...
    with patch('redis.Redis.hget', return_value=b'test_value'), \
...
class TestServerUtilityFunctions:
    ...
    def test_player_join_room(self, mock_socketio, mock_join_room, mock_storage, mock_request_context, app_context):
        ...
        # Call function
        with patch('polyclash.server.request', mock_request_context), patch('polyclash.server.storage', mock_storage):
    ...
    @patch('polyclash.server.join_room')
    @patch('polyclash.server.socketio')
    def test_player_join_room_all_joined(self, mock_socketio, mock_join_room, mock_storage, mock_request_context, app_context):
    ...
    @patch('polyclash.server.join_room')
    @patch('polyclash.server.socketio')
    def test_viewer_join_room(self, mock_socketio, mock_join_room, mock_storage, mock_request_context, app_context):
...
class TestAPIDecorator:
    ...
    def test_api_call_decorator_server_token(self, mock_storage, app_context):
        ...
        def test_api_call(f):
            ...
            def decorated_function(*args, **kwargs):
                if request.get_json() and request.get_json().get('token') == server_token:
    ...
    def test_api_call_decorator_invalid_server_token(self, mock_storage, app_context):
        ...
        with self.test_app.test_client() as client:
            response = client.post('/test_route2', json={"token": "invalid_" + server_token})
    ...
    def test_api_call_decorator_player_token(self, mock_storage, app_context):
        ...
        def test_api_call(f):
            @wraps(f)
            def decorated_function(*args, **kwargs):
                if request.get_json() and request.get_json().get('token') == "player_token":
    ...
    def test_api_call_decorator_invalid_player_token(self, mock_storage, app_context):
        ...
        with self.test_app.test_client() as client:
            response = client.post('/test_route4', json={"token": "invalid_player_token"})
...
class TestAPIEndpoints:
    ...
    def test_join_black(self, mock_player_join_room, mock_join_room, client, mock_storage, app_context):
        ...
        with patch('polyclash.server.request', MagicMock(sid="test_sid")):
            response = client.post('/sphgo/join', json={"token": server_token, "role": "black", "game_id": "test_game_id"})
    ...
    def test_join_viewer(self, mock_viewer_join_room, mock_join_room, client, mock_storage, app_context):
        ...
        with patch('polyclash.server.request', MagicMock(sid="test_sid")):
            response = client.post('/sphgo/join', json={"token": server_token, "role": "viewer", "game_id": "test_game_id"})
    ...
    def test_join_invalid_role(self, client, mock_storage, app_context):
        ...
        response = client.post('/sphgo/join', json={"token": server_token, "role": "invalid_role"})
    ...
    def test_play_valid(self, mock_socketio, client, mock_storage, app_context):
        ...
        # Make a request to the endpoint
        response = client.post('/sphgo/play', json={
            "token": "player_token",
            "steps": 0,
            "play": valid_play
        })
    ...
    def test_play_wrong_turn(self, client, mock_storage, app_context):
        ...
        mock_storage.get_game_id.return_value = "test_game_id"
        mock_storage.exists.return_value = True
        mock_storage.get_role.return_value = "white"  # White can't play on even steps (0)
...
class TestSocketEvents:
    ...
    def test_on_ready_black(self, mock_thread, mock_emit, mock_delayed_start, mock_storage):
