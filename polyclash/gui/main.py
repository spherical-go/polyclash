...
from PyQt5.QtWidgets import QAction, QMainWindow, QMenu, QMessageBox, qApp
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None, controller=None):
        ...
        self.overlay_info.setGeometry(
            self.margin, self.margin, self.width() // 6, self.width() // 6
        )  # x, y, width, height

        self.overlay_map = OverlayMap(self)
        self.overlay_map.setGeometry(
            self.width() // 4 * 3 - self.margin,
            self.margin,
            self.width() // 4,
        ...
        self.sphere_view = ActiveSphereView(
            self.frame, controller, self.status_bar, self.overlay_info, self.overlay_map
        )
    ...
    def handle_network_notification(self, event, data):
        ...
        if event == "ready":
            self.status_bar.showMessage(
                f"{data['role'].capitalize()} player is ready..."
            )
            return
    ...
    def updateOverlayPosition(self):
        ...
        self.overlay_info.setGeometry(
            self.margin, self.margin, overlay_width, overlay_height
        )
        overlay_width = self.width() // 4
        overlay_height = self.height() - 4 * self.margin
        self.overlay_map.setGeometry(
            self.width() - overlay_width - self.margin,
            self.margin,
            overlay_width,
            overlay_height,
        )
