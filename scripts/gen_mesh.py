...
# 根据 ξ 和 φ 计算起始顶点 p
p = np.array(
    [phi**2 * (1 - xi), -(phi**3) + phi * xi + 2 * phi * xi**2, xi], dtype=np.float64
)
...
while True:
    ...
    new_vertices = [
        u
        for u in g
        if not np.any([np.all(np.isclose(v, u, atol=tolerance)) for v in vertices])
    ]
...
# 识别三角形
triangles = [
    list(triangle) for triangle in nx.enumerate_all_cliques(G) if len(triangle) == 3
]
...
for triangle in triangles:
    edge_counts.update(
        ...
        + [(triangle[(i + 1) % 3], triangle[i]) for i in range(3)]
    )
...
# Create the mesh
# 准备用于 PyVista 的面数据
faces_list = [[3] + triangle for triangle in triangles] + [
    [5] + pentagon for pentagon in pentagons
]
