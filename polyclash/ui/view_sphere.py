...
class ActiveSphereView(QtInteractor):
    ...
    def update_maps_view(self):
        img0 = get_hidden().capture_view(0, 5 * axis[0], axis[0], axis[1])
        self.overlay_map.set_image(0, 0, img0)
        img1 = get_hidden().capture_view(1, 5 * axis[1], axis[1], axis[2])
        self.overlay_map.set_image(1, 0, img1)
        img2 = get_hidden().capture_view(2, 5 * axis[2], axis[2], axis[3])
        self.overlay_map.set_image(2, 0, img2)
        img3 = get_hidden().capture_view(3, 5 * axis[3], axis[3], axis[0])
        self.overlay_map.set_image(3, 0, img3)
        img4 = get_hidden().capture_view(4, 5 * axis[4], axis[4], axis[5])
        self.overlay_map.set_image(0, 1, img4)
        img5 = get_hidden().capture_view(5, 5 * axis[5], axis[5], axis[6])
        self.overlay_map.set_image(1, 1, img5)
        img6 = get_hidden().capture_view(6, 5 * axis[6], axis[6], axis[7])
        self.overlay_map.set_image(2, 1, img6)
        img7 = get_hidden().capture_view(7, 5 * axis[7], axis[7], axis[4])
...
class PassiveSphereView(QtInteractor):
    ...
    def capture_view(self, ix, camera_position, camera_focus, camera_up):
        file_screenshot = f"test_{ix}.png"
        self.camera.position = camera_position
        self.camera.focal_point = camera_focus
        self.camera.view_up = camera_up
        self.update()
        img = self.screenshot(file_screenshot, transparent_background=True, scale=2, return_img=True)
        return qimage(img)
