...
class TestAPIFunctions:
    ...
    def test_connect(self, mock_post):
        ...
        black_key, white_key, viewer_key = connect(
            "http://test-server.com", "test_token"
        )

        # Verify the function called requests.post with the correct arguments
        mock_post.assert_called_once_with(
            "http://test-server.com/sphgo/new", json={"token": "test_token"}
        )
    ...
    def test_play(self, mock_post):
        ...
        mock_post.assert_called_once_with(
            ...
            json={"token": "test_token", "steps": 1, "play": [0, 1, 2]},
        )
    ...
    def test_close(self, mock_post):
        ...
        close("http://test-server.com")
        mock_post.assert_called_once_with(
            "http://test-server.com/sphgo/close", json={"token": "test_token"}
        )
